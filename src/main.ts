...
document.addEventListener("DOMContentLoaded", () => {
      ...
      chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
        if (tabs[0].id && tabs[0].url) {
          if (isWhitelisted(tabs[0].url)) {
            status.textContent = "Extension disabled for this whitelisted domain.";
  ...
  function updateSelectorTags() {
    chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
      ...
      const allTags = Object.entries(selectorMap)
        .filter(([domain]) => {
          return ["general", new URL(tabs[0].url!).hostname].includes(domain);
        })
        .map(([domain, selectors]) => {
          return selectors
            .map((selector) => {
              ...
              const label =
                domain === "general" ? `${selector}` : `{${domain}} ${selector}`;
      ...
      document.querySelectorAll(".remove-button").forEach((button) => {
        button.addEventListener("click", (e: Event) => {
          ...
          const selectorToRemove = target.getAttribute("data-selector");
          if (domain && selectorToRemove) {
            ...
            if (domain !== "general" && selectorMap[domain].length === 0) {
  ...
  addForAllButton.addEventListener("click", () => {
    ...
    selectorMap.general = [...new Set([...selectorMap.general, ...newSelectors])];
    updateSelectorTags();
    saveSelectorMap();
  ...
  addForCurrentButton.addEventListener("click", () => {
    chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
      ...
      selectorMap[hostname] = [...new Set([...selectorMap[hostname], ...newSelectors])];
      updateSelectorTags();
      saveSelectorMap();
      selectorsTextarea.value = "";
      refreshCurrentTab();
  ...
  function updateWhitelistTags() {
    whitelistTagsDiv.innerHTML = whitelist
      ...
      .join("");
    document.querySelectorAll("#whitelist-tags .remove-button").forEach((button) => {
      button.addEventListener("click", (e: Event) => {
        const domainToRemove = (e.currentTarget as HTMLButtonElement).getAttribute(
          "data-domain",
        );
        if (domainToRemove) {
